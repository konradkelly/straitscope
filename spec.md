...
> Dover and Gibraltar's corridors were recalibrated 2026-07-09 against ~2 days of live production data (see `geo.js` comments on each region for the numbers). The eyeballed originals traced a narrow "true TSS lane" width, which left 47% (Dover) / 27.6% (Gibraltar) of positions classified `outside` either corridor — starving `classifyRoute`'s 70% threshold and producing the 96.7%-"mixed" route split observed in prod. Splitting positions by COG (NE/SW-bound for Dover, E/W-bound for Gibraltar) showed the two directions *do* separate cleanly by latitude, so the corridor separator line is data-derived from that crossover — but the outer bounds are pushed out to the ROI edge rather than tracing a realistic ~2nm lane width, because real traffic (anchorages, port approaches, non-lane-following vessels) disperses well beyond a true lane; a narrow redraw was tried first and made `outside` *worse* (24-44%) than the wide-placeholder original in some cases. Net effect: `northern`/`southern` mean "which half of the strait", not a precise IMO lane trace — consistent with how the feature was already documented. Gates were not recalibrated this pass.
