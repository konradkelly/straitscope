...
// West gate: lon 55.7, lat 25.9–26.55. East gate: lon 57.1, lat 25.3–26.1.
// (see src/geo.js GATES)
...
function feed(state, points) {
  let s = state;
  let lastResult = { state: s, transit: null };
  for (const p of points) {
    lastResult = applyPosition(s, { time: new Date(p.time ?? Date.now()), lat: p.lat, lon: p.lon, sog: p.sog ?? 10, corridor: p.corridor ?? 'outside' });
    s = lastResult.state;
  }
  return lastResult;
}
...
test('IDLE + no gate crossing stays IDLE and just tracks position', () => {
  ...
  const { state, transit } = applyPosition(s, { time: new Date(), lat: 27.0, lon: 55.6, sog: 5, corridor: 'outside' });
...
test('IDLE -> IN_STRAIT on west gate crossing', () => {
  ...
  const withPos1 = applyPosition(s, { time: new Date(1000), ...p1, sog: 10, corridor: 'outside' }).state;
  const { state, transit } = applyPosition(withPos1, { time: new Date(2000), ...p2, sog: 10, corridor: 'outside' });
  assert.equal(state.state, 'IN_STRAIT');
  assert.equal(state.entered_gate, 'west');
  assert.equal(transit, null);
});

test('full outbound transit: enter west, exit east -> outbound', () => {
  const s = freshState(1);
  const t0 = Date.now();
  const points = [
  ...
  const { state, transit } = feed(s, points);
  assert.equal(state.state, 'IDLE');
  assert.ok(transit);
...
test('full inbound transit: enter east, exit west -> inbound', () => {
  const s = freshState(1);
  const t0 = Date.now();
  const points = [
...
test('a fresh position always clears dark_flagged', () => {
  const s = { ...freshState(1), state: 'IN_STRAIT', dark_flagged: true, last_lat: 26.2, last_lon: 56.0 };
  const { state } = applyPosition(s, { time: new Date(), lat: 26.2, lon: 56.1, sog: 5, corridor: 'northern' });
  assert.equal(state.dark_flagged, false);
});

test('classifyRoute: northern >= 70% threshold', () => {
  assert.equal(classifyRoute({ northern_count: 8, southern_count: 1, total_count: 10 }), 'northern');
});

test('classifyRoute: southern >= 70% threshold', () => {
  assert.equal(classifyRoute({ northern_count: 1, southern_count: 8, total_count: 10 }), 'southern');
});

test('classifyRoute: below threshold on both sides is mixed', () => {
  assert.equal(classifyRoute({ northern_count: 5, southern_count: 4, total_count: 10 }), 'mixed');
});

test('classifyRoute: zero positions is mixed', () => {
  assert.equal(classifyRoute({ northern_count: 0, southern_count: 0, total_count: 0 }), 'mixed');
});
